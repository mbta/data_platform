defmodule ExCubicIngestion.Workers.ErrorTest do
  ...
  describe "perform/1" do
    ...
    test "run job for load with metadata file", %{
      ...
      assert "errored" == CubicLoad.get!(ods_load.id).status
    end
    ...
    test "run job for load with metadata file that doesn't exists", %{
      ...
      assert "errored_unknown" == CubicLoad.get!(ods_load_metadata_unknown.id).status
    end
  end
  ...
  describe "construct_destination_key_root/1" do
    test "getting destination key for generic load", %{
      ...
      assert "cubic/dmap/sample/20220101" == Error.construct_destination_key_root(dmap_load)
    end

    test "getting destination key for ODS load", %{
      ods_load: ods_load
    } do
      assert "cubic/ods_qlik/SAMPLE/timestamp=20220101T204950Z/LOAD1" ==
               Error.construct_destination_key_root(ods_load)
    end
  end
end
